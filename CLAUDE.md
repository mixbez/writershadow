...
## ⚠️ КРИТИЧНО: Обновление README.md

**Перед каждым обновлением README.md — сохрани текущую версию как OLDREADME.md:**

```bash
cp README.md OLDREADME.md
```

Затем обновляй README.md.

В репозитории всегда должны лежать оба файла:
- **README.md** — актуальное состояние (как должно работать сейчас)
- **OLDREADME.md** — предыдущая рабочая версия (страховка на случай если что-то сломали)

Обновлять README нужно только когда всё работает и проверено. OLDREADME не трогать — он перезаписывается автоматически при следующем обновлении README.
